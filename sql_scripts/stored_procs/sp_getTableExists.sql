...
--*         to check if target table exists                             *-
--*                                                                     *-
--**********************************************************************--

CREATE OR REPLACE PROCEDURE `metadata_utilities`.sp_getTableExists(
  param1 STRING, 
  param2 STRING,
  param3 STRING
  ) 
BEGIN 
  ...
  -- Build Query 
    SET v_query = 'SELECT COUNT(1) AS flag FROM `' || v_project_id || '.' || v_dataset || '.__TABLES_SUMMARY__' || '` WHERE table_id = "' || v_table_name || '"' ;
